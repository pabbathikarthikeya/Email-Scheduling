...
def analyze_and_notify_crew():
    ...
    for crew_id, crew_data in all_crew_data.items():
        ...
        # ========== MODIFICATION START ==========
        # This condition now only triggers if the expired_certs list is not empty.
        if expired_certs:
            ...
            # The prompt is simplified to only mention expired documents.
            prompt = f"""
            ...
            print(f"ℹ️ Skipping email for {name}: No expired certifications found.")
